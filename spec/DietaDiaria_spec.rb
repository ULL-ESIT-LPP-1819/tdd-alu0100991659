...
RSpec.describe DietaDia do
    ...
    it "Todo funciona correctamente" do
        puts @menu.to_s
        expect(@menu.kcal).to eq(538.82)
        expect(@menu.kcal == @menu.kcal).to eq(true)
        @a = Individuos.new("Peter", false, true, 73, 1.82, 25, 1, 0.12)
        expect(@menu.is_enough(@a)).to eq(false)
        @prueba = DietaDia.new("Domingo")
        expect(@prueba.nombre).to eq("Domingo")
    end
end